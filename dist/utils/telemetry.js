...
export class Telemetry {
    ...
    endOperation(id, metadata) {
        ...
        this.logger.debug(`Operação finalizada: ${metric.operation}`, { id, duração: `${durationMs}ms`, success: metric.success, metadata });
    }
    /**
     ...
     */
    failOperation(id, error, metadata) {
        const metric = this.activeOperations.get(id);
        if (!metric) {
        ...
        this.addMetric(metric);
        const durationMs = metric.duration;
        this.logger.error(`Operação falhou: ${metric.operation}`, { id, duração: `${durationMs}ms`, error: error.message, metadata });
    }
    /**
    ...
    getOperationDuration(id) {
        ...
        const finishedMetric = this.metrics.find(metric => metric.metadata && 'id' in metric.metadata && metric.metadata.id === id);
