...
async function startPiperunMcpServer() {
  try {
    ...
    // Deal tools
    server.setRequestHandler(
      createMethodWithParamsSchema(dealTools.listDeals.name, dealTools.listDeals.schema),
      adaptObjectParamsHandler(dealTools.listDeals.handler)
    );
    server.setRequestHandler(
      createMethodWithParamsSchema(dealTools.getDealDetails.name, dealTools.getDealDetails.schema),
      adaptNamedParamsHandler(dealTools.getDealDetails.handler)
    );
    server.setRequestHandler(
      createMethodWithParamsSchema(dealTools.updateDeal.name, dealTools.updateDeal.schema),
      adaptNamedParamsHandler(dealTools.updateDeal.handler)
    );
    
    // Pipeline tools
    server.setRequestHandler(
      createMethodWithParamsSchema(pipelineTools.listPipelines.name, pipelineTools.listPipelines.schema),
      adaptObjectParamsHandler(pipelineTools.listPipelines.handler)
    );
    server.setRequestHandler(
      createMethodWithParamsSchema(pipelineTools.listStages.name, pipelineTools.listStages.schema),
      adaptObjectParamsHandler(pipelineTools.listStages.handler)
    );
    
    // Product tools
    server.setRequestHandler(
      createMethodWithParamsSchema(productTools.listProducts.name, productTools.listProducts.schema),
      adaptObjectParamsHandler(productTools.listProducts.handler)
    );
    ...
    // Stats tools
    server.setRequestHandler(
      createMethodWithParamsSchema(statsTools.getServerStats.name, statsTools.getServerStats.schema),
      adaptHandler(statsTools.getServerStats.handler)
    );
    server.setRequestHandler(
      createMethodWithParamsSchema(statsTools.checkHealth.name, statsTools.checkHealth.schema),
      adaptHandler(statsTools.checkHealth.handler)
    );
    ...
    // Deal prompts
    server.setRequestHandler(
      createMethodWithParamsSchema(dealPrompts.analyzeDeal.name, dealPrompts.analyzeDeal.schema),
      adaptNamedParamsHandler(dealPrompts.analyzeDeal.handler)
    );
    server.setRequestHandler(
      createMethodWithParamsSchema(dealPrompts.summarizeDeals.name, dealPrompts.summarizeDeals.schema),
      adaptObjectParamsHandler(dealPrompts.summarizeDeals.handler)
    );
    
    // Pipeline prompts
    server.setRequestHandler(
      createMethodWithParamsSchema(pipelinePrompts.analyzePipeline.name, pipelinePrompts.analyzePipeline.schema),
      adaptNamedParamsHandler(pipelinePrompts.analyzePipeline.handler)
    );
    server.setRequestHandler(
      createMethodWithParamsSchema(pipelinePrompts.compareStages.name, pipelinePrompts.compareStages.schema),
      adaptNamedParamsHandler(pipelinePrompts.compareStages.handler)
    );
    
    // Contact prompts
    server.setRequestHandler(
      createMethodWithParamsSchema(contactPrompts.analyzeContact.name, contactPrompts.analyzeContact.schema),
      adaptNamedParamsHandler(contactPrompts.analyzeContact.handler)
    );
    server.setRequestHandler(
      createMethodWithParamsSchema(contactPrompts.identifyHighValueContacts.name, contactPrompts.identifyHighValueContacts.schema),
      adaptNamedParamsHandler(contactPrompts.identifyHighValueContacts.handler)
    );
