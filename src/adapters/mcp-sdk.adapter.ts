...
// Tipos exportados para uso no restante do código
...
export class ResourceTemplate {
  constructor(public baseUrl: string, public paths: Record<string, string | undefined> = {}) {}
}

// Interface para registros de ferramentas, recursos e prompts
...
export class MCPServerWrapper {
    ...
    if (this._server.setRequestHandler) {
      try {
        ...
        if (this._server.sendToolListChanged) {
          this._server.sendToolListChanged().catch((error: Error) => {
   ...
   */
  private _registerAllComponents(): void {
 ...
 */
export async function initializeMcpSdk(config: { name: string, version: string }) {
  try {
    ...
    const { 
      ...
      ResourcesListSchema, 
      PromptsListSchema 
    } = await import('../utils/mcp-schema.util.js');
    ...
    return {
      ...
      async initializeHttpTransport(endpoint = '/mcp') {
        try {
          ...
          return {
            createTransport(res?: any) {
              return new SSEServerTransport(endpoint, res || {}) as any; // Type assertion para evitar erro de tipo
            }
          };
        } catch (error) {
